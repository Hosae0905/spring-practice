...
### 빈 생명주기 콜백 시작
- **데이터베이스 커넥션 풀**이나 **네트워크 소켓**처럼 애플리케이션 시작 시점에 필요한 연결을 미리 해두고 애플리케이션 종료 시점에 연결을 모두 종료하는 작업을 진행하려면 객체의 초기화와 종료 작업이 필요하다.
- **객체의 초기화와 종료 작업이 필요하다.**
- 스프링 빈은 간단하게 다음과 같은 라이프사이클을 가진다.
  - **객체 생성 --> 의존관계 주입**
- 스프링 빈은 객체를 생성하고 **의존관계 주입**이 다 끝난 다음에야 필요한 데이터를 사용할 수 있는 준비가 된다.
...
### 인터페이스(InitializationBean, DisposableBean)
- 초기화, 소멸 인터페이스의 단점
  - 스프링 전용 인터페이스이기 때문에 **의존적**으로 개발할 수밖에 없다.
  - 초기화, 소멸 **메서드 이름을 변경할 수 없다.**
  - 내가 코드를 고칠 수 없는 **외부 라이브러리에 적용할 수 없다.**
  - 스프링 초창기에 나온 방법이므로 지금은 더 나은 방법이 있어서 거의 사용하지 않는 방법이다.
...
- **자바 표준 기술**이기 때문에 스프링이 아닌 다른 컨테이너에서도 동작한다.
- 컴포넌트 스캔과 잘 어울린다.
- 단점으로는 **외부 라이브러리에 적용하지 못한다는 것**이다. 외부 라이브러리를 초기화, 종료해야 한다면 **@Bean**의 기능을 사용하자.
